...
class _ShapePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final meanStroke = Paint()
      ..color = Colors.white.withOpacity(showSpecimen ? 0.50 : 0.95) // 50% si hay superposición
      ..strokeWidth = 1.2
      ..style = PaintingStyle.stroke;

    final meanDot = Paint()
      ..color = Colors.white.withOpacity(showSpecimen ? 0.50 : 0.95);

    final specStroke = Paint()
      ..color = const Color(0xFF9AD5B0) // verde suave
      ..strokeWidth = 1.2
      ..style = PaintingStyle.stroke;

    final specDot = Paint()..color = const Color(0xFF9AD5B0);
  ...
  Rect _bounds(List<Offset> pts) {
    ...
    if ((maxX - minX).abs() < 1e-9) { minX -= 0.5; maxX += 0.5; }
    if ((maxY - minY).abs() < 1e-9) { minY -= 0.5; maxY += 0.5; }
