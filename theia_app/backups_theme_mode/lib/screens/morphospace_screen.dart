...
class _MorphospaceScreenState extends State<MorphospaceScreen> {
  ...
  void initState() {
    super.initState();
    _rowKeys = List<GlobalKey>.generate(widget.pcaScores.length, (_) => GlobalKey());
  }
  ...
  Widget build(BuildContext context) {
    ...
    final spotsWithIndex = widget.pcaScores.asMap().entries.map((e) {
      ...
      final color = isSel
          ? Colors.redAccent
          : (isFaded ? baseColor.withOpacity(0.28) : baseColor);
    ...
    String hdr(String base, int i) =>
        exp != null && exp.length > i ? '$base (${exp[i].toStringAsFixed(1)}%)' : base;

    return Scaffold(
      appBar: AppBar(
        title: const Text('Morfoespacio (PC1 vs PC2)'),
        actions: [
          if (_selectedIndex != -1)
            IconButton(
                ...
                ScatterChartData(
                  ...
                  titlesData: FlTitlesData(
                    ...
                    bottomTitles: AxisTitles(
                      axisNameWidget: const Text('PC1 (Eje X)  /  PC2 (Eje Y)', style: TextStyle(color: Colors.white)),
                      sideTitles: SideTitles(
                        showTitles: true,
                        reservedSize: 24,
                        interval: bounds.interval,
                        getTitlesWidget: (value, meta) {
                          if (value == bounds.maxX || value == bounds.minX) return const SizedBox.shrink();
                          return Text(value.toStringAsFixed(1), style: const TextStyle(color: Colors.white, fontSize: 12));
                        },
                      ),
                    ),
                    leftTitles: AxisTitles(
                      sideTitles: SideTitles(
                        ...
                        interval: bounds.interval,
                        getTitlesWidget: (value, meta) {
                          if (value == bounds.maxY || value == bounds.minY) return const SizedBox.shrink();
                          return Text(value.toStringAsFixed(1), style: const TextStyle(color: Colors.white, fontSize: 12));
                        },
                      ),
                    ),
                  ...
                  scatterTouchData: ScatterTouchData(
                    ...
                    touchTooltipData: ScatterTouchTooltipData(
                      getTooltipItems: (spot) {
                        ...
                        final name = widget.pcaScores[idx]['image_name'] ?? 'N/A';
                    ...
                    touchCallback: (event, resp) {
                      ...
                      if (event is FlTapUpEvent) {
                        ...
                        if (idx != -1) {
                          ...
                          WidgetsBinding.instance.addPostFrameCallback((_) => _scrollToRow(idx));
                        }
                      }
                    },
                  ),
  ...
  Widget _buildScoresTable({required String headerPc1, required String headerPc2}) {
